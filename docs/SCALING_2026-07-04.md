...
| Rate limit | `src/shared/security/rate-limit.ts` | Redis when `UPSTASH_*` or `KV_REST_*` set |
...
| Priority | Action |
|----------|--------|
| P0 | **Redis required** in production for distributed rate limiting |
| P0 | **Neon compute tier up**; Neon region aligned with Vercel function region |
| P1 | Fill Anam pool (5–10 slots); monitor 429 / quota errors |
...
- [ ] **Redis / Upstash** — set in Vercel production:
  - `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN`, **or**
  - `KV_REST_API_URL` + `KV_REST_API_TOKEN` (Vercel KV)
  - Without Redis, rate limits are per-instance only (`src/shared/security/rate-limit.ts`).
...
- [ ] **Anam pool** — configure 5–10 of `ANAM_API_KEY` … `ANAM_API_KEY_10` in Vercel; verify with `npm run providers:status`.
