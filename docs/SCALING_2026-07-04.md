...
flowchart TB
  ...
  subgraph data [Data and async]
    ...
    Redis[(Upstash Redis via Vercel Storage)]
...
| Rate limit | `src/shared/security/rate-limit.ts` | Redis when Vercel Storage / Upstash is linked (`src/shared/config/redis-rest.ts`) |
| Brain rate limit | `src/features/runtime-session/lib/brain-stream-rate-limit.ts` | 40 req/min per `userId:employeeId` |
| Anam pool | `src/shared/config/anam-api-pool.ts` | `ANAM_API_KEY` … `ANAM_API_KEY_10` |
| Billing limits | `src/features/billing/config/plans.ts` | Free 120s Talk, chunk caps |
...
| Rate limit | Redis recommended | See [Ops checklist](#6-operations-checklist) |
...
| Priority | Action |
|----------|--------|
| P0 | **Vercel Storage → Redis (Upstash)** linked to project (auto env; see [Ops checklist](#6-operations-checklist)) |
...
| Plan | Employees | Talk session | Knowledge chunks |
|------|-----------|--------------|------------------|
| Free | 1 | 120 s | 5,000 |
| Super Pro | Unlimited | Unlimited | 32,000 |
...
- Talk minutes / month metering (`employee_session`)
...
| Anam | Concurrent streams | Enterprise contract; do not rely on 10 lab slots alone |
...
- [ ] **Redis (rate limits)** — in Vercel: **Storage → Redis (Upstash) → Connect to project**. Credentials are injected automatically; do not paste REST URL/token manually. Verify: `npm run providers:status` → `Redis (rate limit): OK`.
  - Without Storage link, rate limits are per-instance only (`src/shared/security/rate-limit.ts`).
...
- [ ] **Anam pool** — keys already in Vercel (`ANAM_API_KEY` … `ANAM_API_KEY_10`); verify pool health with `npm run providers:status` and admin Anam pool view.
...
| Talk session duration cap | Implemented | `sessionLimitSeconds`, free = 120s |
...
| Talk minutes / month | **Future** | Phase C |
...
| Talk minutes metering | COGS >30% revenue | Aggregate `employee_session.duration_seconds` per org/month |
